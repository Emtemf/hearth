...
function ControlRoom() {
  ...
  useEffect(() => {
    if (!selected) return
    const source = connectEvents(selected, () => {
      void Promise.all([
        queryClient.invalidateQueries({ queryKey: ['sessions'] }),
        queryClient.invalidateQueries({ queryKey: ['transcript', selected] }),
        queryClient.invalidateQueries({ queryKey: ['invocations', selected] }),
        queryClient.invalidateQueries({ queryKey: ['exchanges', selected] }),
      ])
    })
    return () => source.close()
  }, [selected])
